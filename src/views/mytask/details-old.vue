<template>
  <div class="operation-content">
    ...
    >
      <div class="text-content operate-box">
        <div class="text-header">
          <div class="text-header__content">
            ...
            <p class="title" v-if="taskInfo.status == CONFIG.ORDER_STATUS_FAIL">
              该任务审核失败
            </p>
      ...
      <div class="tb-order" v-if="taskInfo.tkl_checked != 1">
        ...
        >
          ...
          <input-box :inputNum="CONFIG.TB_ORDER_NUM" ref="InputBox"></input-box>
        </div>
        <div
          class="tb-order_price"
          v-if="
      ...
      <div
        ...
        "
      >
        <div class="tb-word-header">请粘贴该商品的淘口令，用于核对商品链接</div>
    ...
    <!-- <div class="my-btn" @click="goTaskList">我的任务</div> -->
    <fixed-btn :urlInfo="backForm" ref="fiexdbtns" />
  </div>
</template>

<script>
import { getTaskInfoHandler, commitTaskHandler, checkProductUriHandler } from '@/api/operation'
...
export default {
  ...
  methods: {
    ...
    async getTask () {
      let _this = this
      let params = { task_id: _this.qrOrderId }
      ...
      if (data.code === _this.CONFIG.CODE_SUCCESS) {
        _this.taskInfo = data.result
        _this.isTask = true
        let lSecond = (new Date().getTime() - new Date((data.result.gmt_update).replace(/-/g, '/')).getTime())
    ...
    goTaskList () {
      var _this = this
      _this.$router.push({ path: '/mytask', query: { qr_order_id: _this.qrOrderId, page_name: '/taskdetails' } })
    }
  }
}
</script>
