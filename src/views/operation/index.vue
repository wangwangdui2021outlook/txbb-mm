...
export default {
  ...
  methods: {
    ...
    bank_card_list(){
      let _this = this
      _this.bankSlots[0].values = []
      _this.bankSlots[0].cards = [{
        bankName: "中国邮政储蓄银行",
        bankCode: "PSBC",
        patterns: [{
      ...
      _this.bankSlots[0].cards.forEach((elem, index) => {
        _this.bankSlots[0].values.push(elem.bankName)
      });
    },
    // 银行卡选择
    ...
    async setUserRefundInfo () {
      ...
      let bank_card_num = params.bank_card_num.replace(/ /g,'')
      if(bank_card_num.length == 16 || bank_card_num.length == 19 ){
        _this.bankSlots[0].cards.forEach((elem, index) => {
          _this.bankSlots[0].values.push(elem.bankName)
          elem.patterns.forEach((elem1, index1) => {
            if(elem1.reg.test(bank_card_num) && elem1.cardType == 'DC'){
              _this.bankInfo.bank_card_num = elem.bankName
            }
          })
        });
      } else if(bank_card_num == '') {
          return MessageBox.alert('无效账号！')
      } else {
        return MessageBox.alert('无效账号！')
      }
